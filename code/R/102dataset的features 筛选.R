##特征筛选######
compositiontransition=function(aminoseq1)
{
x=aminoseq1
xamino=strsplit(x,"")
xamino1=xamino[[1]]
...
feature.name78=c(paste(c("x"),c(1:104),sep=""))
#################420个特征######
...
{
train.aa=data$AA
train.xname=data$Entryname                   
train.class2=log10(data$fold)
#############################
################################
################################
#建立数据框
...
#######合并数据矩阵#########
train.102matrix=cbind(train.420matrix,train.matrix)
dim(train.102matrix)
####直接提取mrmr特征
data.mrmr=read.table("F:/zhulin/1datasz1/RNA/mrmrorderfeature.csv",header=TRUE,sep=",")
featurename=data.mrmr$featurename
col=colnames(train.102matrix)
site=match(featurename,col)
train.102matrix182=train.102matrix[,site]
colnames(train.102matrix182)

##Pls分析
spls.train.182feature=plsr(train.class2~train.102matrix182,ncomp=10,scale=TRUE)
plot(RMSEP(spls.train.182feature), legendpos = "topright")
plot(spls.train.182feature,line=TRUE)
plot(R2(spls.train.182feature),legendpos = "topright")
plot(RMSEP(spls.train.182feature), legendpos = "topright")
x=RMSEP(spls.train.182feature)
rmse=as.numeric(x$val)
nrmse=as.numeric(x$val)/sd(train.class2)
plot(nrmse,type='l',main='NRMSE')
for(i in 1:10)
{
pearson[i]=cor(spls.train.182feature[["fitted.values"]][,,i],train.class2)
spearman[i]=cor(spls.train.182feature[["fitted.values"]][,,i],train.class2,method = "spearman")
}
pearson
spearman

table=data.frame(Fold=train.class2,predict=spls.train.182feature[["fitted.values"]][,,10])
write.table(table,file="F:/zhulin/1datasz1/RNA/91-181.csv",sep=",",row.names=F,col.names=T,quote=F)
...
#Y.label='Component 2',title='train-102-new-182features',plot.title = element_text(hjust = 0))
...
#################
test.predict=predict(spls.train.182feature,test.102matrix182,dist="max.dist")
...
colnames(test.matrix)=feature.name78
colnames(train.matrix)=feature.name78
#######去处全部为0的特征变量#########
...
#Y.label='Component 2',title='train-102-new-78features',plot.title = element_text(hjust = 0))
